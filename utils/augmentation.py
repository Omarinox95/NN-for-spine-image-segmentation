...
class Data_Augmentation:
    ...
    config_p_per_sample = 0.50  # Probability a data augmentation technique will be performed on the sample
    config_mirror_axes = (1,)
    config_contrast_range = (0.3, 3.0)
    config_brightness_range = (0.5, 2)
    config_gamma_range = (0.7, 1.5)
    config_gaussian_noise_range = (0.0, 0.05)
    config_elastic_deform_alpha = (0.0, 100.0)
    config_elastic_deform_sigma = (9.0, 13.0)
    config_rotations_angleX = (-5. / 360 * 2. * np.pi, 5. / 360 * 2. * np.pi)
    config_rotations_angleY = (-5. / 360 * 2. * np.pi, 5. / 360 * 2. * np.pi)
    config_rotations_angleZ = (-5. / 360 * 2. * np.pi, 5. / 360 * 2. * np.pi)
    ...
    def run(self, img_data, seg_data):
        ...
        if self.rotations or self.scaling or self.elastic_deform or \
            ...
            if self.cropping:
                patch_shape = self.cropping_patch_shape
            else:
                
                patch_shape = img_data[0].shape[-2:]
        ...
        # Data structure channel-first 3D:  (batch, channel, x, y)
        img = aug_img_data
        mask = aug_seg_data
